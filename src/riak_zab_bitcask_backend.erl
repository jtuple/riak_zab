%% -------------------------------------------------------------------
%%
%% riak_zab_bitcask_backend: Copy of riak_kv_bitcask_backend to avoid
%%                           requiring entire riak_kv app as a
%%                           dependency. Changed application:get_env
%%                           from bitcask to riak_zab.
%%
%% -------------------------------------------------------------------
%%
...
-module(riak_zab_bitcask_backend).
-behavior(riak_zab_backend).
...
start(Partition, Config) ->
    ...
    DataDir =
        case proplists:get_value(data_root, Config) of
            undefined ->
                case application:get_env(riak_zab, data_root) of
                    {ok, Dir} ->
                        Dir;
                    _ ->
                        riak_zab_app:full_stop("bitcask data_root unset, "
                                               "failing")
                end;
            Value ->
                Value
    ...
    case filelib:ensure_dir(BitcaskRoot) of
        ...
        {error, Reason} ->
            error_logger:error_msg("Failed to create bitcask dir ~s: ~p\n",
                                   [BitcaskRoot, Reason]),
            riak_zab_app:full_stop("riak_zab_bitcask_backend failed to start.")
    end,
...
maybe_schedule_sync(Ref) when is_reference(Ref) ->
    case application:get_env(riak_zab, sync_strategy) of
        {ok, {seconds, Seconds}} ->
...
schedule_sync(Ref, SyncIntervalMs) when is_reference(Ref) ->
    riak_zab_backend:callback_after(SyncIntervalMs, Ref, {sync, SyncIntervalMs}).

schedule_merge(Ref) when is_reference(Ref) ->
    riak_zab_backend:callback_after(?MERGE_CHECK_INTERVAL, Ref, merge_check).
...
simple_test() ->
    ...
    application:set_env(riak_zab, data_root, "test/bitcask-backend"),
    riak_zab_backend:standard_test(?MODULE, []).

custom_config_test() ->
    ?assertCmd("rm -rf test/bitcask-backend"),
    application:set_env(riak_zab, data_root, ""),
    riak_zab_backend:standard_test(?MODULE, [{data_root, "test/bitcask-backend"}]).
